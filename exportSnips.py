...
def lsSnips(table_name, cn):
    ...
    which will then be pasted into the cctbxsnips.db.
    """
    c.execute('SELECT * FROM {tn} WHERE {cn}="source.python"'.
              format(tn=table_name, cn=col_3))
    all_rows = c.fetchall()
...
def writeAtom(scopeAtom, args):
    ...
    """
    selected = args
    home = r'/Users/blaine/454CCTBXsnips/cctbxsnips/atomcctbxsnips/'
...
def writeSnipMate(scopeSnipMate, args):
    ...
    """
    selected = args
    home = '/Users/blaine/454CCTBXsnips/cctbxsnips/snipmatecctbxsnips/'
...
def writeST3all(args):
    ...
    """
    selected = args
    home = '/Users/blaine/454CCTBXsnips/cctbxsnips/st3cctbxsnips/'
    for (tabtrigger,
         language,
         scope,
         category,
...
def writeUS(scopeUS, args):
    ...
    home = '/Users/blaine/454CCTBXsnips/cctbxsnips/ultisnipcctbxsnips/'
    selected = args
...
def writeNeoSnippets(scopeNeoSnippets, args):
    ...
    """
    selected = args
    home = '/Users/blaine/454CCTBXsnips/cctbxsnips/neosnippetscctbxsnips/'
...
def writeVSC(scopeVSC, args):
    ...
    June 7, 2021
    I switched to using the body field in the pymolsnips database. 
    This meant that I had to tack on the terminal tab stop.
    
    Some of the code in the body column has double quotes.
    This conflicts with the required flanking double quotes in the JSON file.
    The double quotes in the body need to be converted to single quotes by regex.
    
    Copy and pasted the code from exportPySnips22July30.py on 27 Janurary 2023.
    """
    selected = args
    home = '/Users/blaine/454CCTBXsnips/cctbxsnips/vsccctbxsnips/'
    ...
    for (tabtrigger,
        ...
        myregex = r'"'
        mysub = r"'"
        body2 = re.sub(myregex, mysub, body0)
        sbody3 = "\\n\\\n".join((numSpaces * " ") + i for i in body2.splitlines())
        outp1 = '"' + tabtrigger + '": {' + '\n'\
            '    "prefix": "' + tabtrigger + '",' + '\n'\
            '    "body": "' + sbody3 + '",' + '\n'\
            '    "caption": "' + caption + '",' + '\n'\
...
def writeYaSnippets(scopeYaSnippets, args):
    """
    Snippets for YaSnippets with Emacs.
    One file per snippet.
    """
    home1 = r'/Users/blaine/454CCTBXsnips/cctbxsnips/'
    home2 = r'yasnippets/'
    home3 = r'python-mode/'
    libpath = home1 + home2 + home3
    libpath2 = home1 + home2 
    if not os.path.exists(os.path.dirname(libpath2)):
        try:
            os.makedirs(os.path.dirname(libpath2))
        except OSError as exc:  # Guard against race condition
    ...
    for (tabtrigger,
         ...
         citekey,
         pageNumber) in selected:
        output1 = open(home1 + home2 + tabtrigger, 'w')
        outp1 = '# -*- mode: snippet -*-' + '\n' \
...
def writeBrackets(scopeBrackets, args):
    ...
    output1 = open(home + 'bracketscctbxsnips.yml', 'w')
    for (tabtrigger,
         language,
         scope,
         category,
...
def writeElyraCCTBX0(scopeJupyterLab, args):
    """
    Writes code for JupyterLab Elyra-snippets ffot CCTBX snippets in Python
    ...
    28 May 2021
    """
    home1 = '/Users/blaine/0454CCTBXsnips/cctbxsnips4/'
    selected = args
    c.execute('SELECT DISTINCT category FROM {} WHERE {cn}="source.python" ORDER BY category'.format(
        table_name, cn=col_3))
    distinct = c.fetchall()
    ...
    for x in distinct:
        ...
        for (tabtrigger,
            ...
            if category == ''.join(x):
                ...
                output1.write('    "tags": [' + '\n')
                output1.write('    "python","' + tabtrigger + '","' + tag0 + '"\n')
                output1.write('    ],' + '\n')
...
def writeElyraCCTBX0plus(scopeJupyterLab, args):
    """
    Writes code for JupyterLab Elyra-snippets ffot CCTBX snippets in Python.
    Includes second copy of code in a block comment. 
    ...
    28 May 2021
    """
    home1 = '/Users/blaine/0454CCTBXsnips/cctbxsnips5/'
    selected = args
    c.execute('SELECT DISTINCT category FROM {} WHERE {cn}="source.python" ORDER BY category'.format(
        table_name, cn=col_3))
    distinct = c.fetchall()
        ...
        for (tabtrigger,
            ...
            if category == ''.join(x):
                ...
                #    output1.close()
                    ...
                    # Wirite out the marked-up code in a block comment.
                    body4a = '      "' + '",\n      "'.join((numSpaces * "") + i for i in outp001.splitlines()) + '"'
                    outp1a = (body4a + "\n")
                    output1.write(outp1a)
                    # Write out the unmarked code.
                    body4b = '      "' + '",\n      "'.join((numSpaces * "") + i for i in body3aa.splitlines()) + '"'
                    outp1b = (body4b + "\n")
                    output1.write(outp1b)
                else:
...
def writeElyraCCTBX(scopeElyra, args):
    """
    ...
    """
    home1 = '/Users/blaine/0454CCTBXsnips/cctbxsnips4/elyracctbxsnips'
    selected = args
    c.execute('SELECT DISTINCT category FROM {} WHERE {cn}="source.python" ORDER BY category'.format(
        table_name, cn=col_3))
    distinct = c.fetchall()
...
if __name__ == '__main__':
    ...
    home1 = r'/Users/blaine/0454CCTBXsnips/'
    home2 = r'cctbxsnips4/'

    #writeVSC(scopeVSC, args=fetchsnips(table_name, cn))
    #writeAtom(scopeAtom, args=fetchsnips(table_name, cn))
    #writeYaSnippets(scopeYaSnippets,args=fetchsnips(table_name, cn))
    ...
    # writeElyraCCTBX0(scopeElyra, args=fetchsnips(table_name, cn))
    writeElyraCCTBX0plus(scopeElyra, args=fetchsnips(table_name, cn))
